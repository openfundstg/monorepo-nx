...
A promoter's account can be switched to a **demo** in the admin panel. `/auth` then carries a
`TmaDemoPack` — a history, a referral list and every config, generated by the server and stored
nowhere — and `demoInterceptor` answers the account's requests on the device from it. Screens
change nothing: they are drawn by the same components from the same contracts.

- **Every endpoint an api service calls is in `DEMO_ROUTES`**, as one of: from the pack, to the
  network (the launch, `/rates`, a pasted jar), acted out on the device (creating a sale, a
  deposit, a top-up — the next page opens and nothing is sent), or refused.
  `demo-routes.const.spec.ts` reads the api services and fails on an endpoint that is not
  listed, because the runtime default — refuse — would otherwise surface in a recording.
- **The server is the boundary, not this.** `DemoReadOnlyGuard` refuses a demo account's writes
  whatever the device does.
- **Priced live.** The sale form, the deposit screen and the top-up offer are re-priced from the
  `rates` slice, or the form would quote one rate beside a dashboard showing another.
- **No socket and no pixel.** `WsService` reports itself connected without opening one — a real
  push would paint real figures over invented ones — and `MetaPixelService.suspend()` keeps a
  promoter's takes out of the ad account measuring their campaign.
- The refusal is `ERRORS.2700`, worded as an ordinary "not available right now": a button
  pressed by mistake on camera must not put the word *demo* on screen.
...
| `demo/`                                                  | a demo account's answers: `DEMO_ROUTES`, `DemoModeService` and the last interceptor in the chain — see *Demo accounts* below                                                                                                                                             |
